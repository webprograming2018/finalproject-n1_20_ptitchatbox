  ...
  <div class="container-conversation">
    <div class="item-header">
      <div style="position:relative">
        <h3>Tin nhắn</h3>
        <h6>Chuyển tiếp cuộc trò chuyện vào messenger</h6>
      </div>
    </div>

    <div v-for="item of getConversations" v-bind:key="item.id" class="item">
      <a @click.prevent="forwardConversation(item.id)">
...
export default {
  ...
  computed: {
    /*getAllConversation() {
      ...
      const latestMessage = this.$store.state.latestMessage;
      const result = [];
      for (let item of this.conversation) {
        if (item.type != this.type) {
          continue;
        }
        if (result.length === this.limmit) {
          break;
        }
        const obj = {};
        const latest = latestMessage[item[".key"]];
        if (!latest) {
          obj["timestamp"] = item.timestamp;
          obj["latest"] = "";
        } else {
          obj["latest"] = this.getLatestMessage(latest);
          obj["timestamp"] = latest.timestamp;
        }
  ...
  methods: {
    async forwardConversation(conversation_id) {
    ...
    handleFriendInfo(conSnap) {
      ...
      if (data.public) {
        ...
        db.ref("public/" + friend).on("value", snap => {
          ...
          const state = userInfo.state ? true : false;
