...
export default {
  ...
  methods: {
    ...
    _scrollDown () {
//      debugger
      if(this.$refs.scrollList.scrollHeight - this.$refs.scrollList.scrollTop > 1000){
        return;
      }
    ...
    async handleScroll(event) {
      if (event.srcElement.scrollTop === 0) {
        const before = this.$refs.scrollList.scrollHeight;
        await this.loadNewMessage();
        const after = this.$refs.scrollList.scrollHeight;
        // load message
        this.$refs.scrollList.scrollTop = after - before >0 ? after - before : 5;
