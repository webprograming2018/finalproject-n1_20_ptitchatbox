...
async function initGroupChat(idUser, change) {
  let dkmh = null;
  let id = null;
  const task = [];
  ...
  if (change.after.exists()) {
    //create and change
    console.log("Join nhóm: " + idUser);
    dkmh = change.after.val();
    id = idUser;
  }

  if (change.before.exists()) {
    // change and delete
    console.log("Dời nhóm: " + idUser);
    const dkmhOld = change.before.val();
    for (let msmh in dkmhOld) {
      const mh = dkmhOld[msmh];
      const conversation_id = getIdConversationFromMaLop(mh);
      task.push(
        db
          .ref("conversation/" + idUser + "/groups/" + conversation_id)
          .remove()
      );
    }
  }
...
async function joinGroups(id, conversation_id, mssv, nameSV, tenMh, nmh) {
  ...
  if (!conversation) {
    ...
    _mem[id] = true;
    task.push(
      db.ref("privatemember/" + conversation_id).update({
        type: "group",
        members: _mem
      })
    );
    ...
    _mem[id] = true;
    task.push(
      db.ref("privatemember/" + conversation_id + "/members").update(_mem)
    );
  }
  ...
  task.push(
    db.ref("/room_names/" + conversation_id + "/members").update(memRoom)
  );
