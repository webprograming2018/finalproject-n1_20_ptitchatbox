        ...
        <div class="user-info">
          ...
          <div class="username">
            <div v-if="!isEditName" class="name">
              ...
              <span style="margin-left: 10px;">{{getBasicInfo.name}}</span>
              <a
                v-if="typeProfile=='private'"
            ...
            <div
              v-if="typeProfile=='public'"
              class="notication-rule"
            >Mọi người đều có thể thấy các thông tin này.</div>
            <div
              v-if="typeProfile=='private'"
              class="notication-rule"
            >Thông tin khởi tạo cuộc trò chuyện ẩn danh.</div>
...
export default {
  ...
  async created() {
    ...
    this.$bindAsObject(
      "privateInfo",
      this.$store.state.db.ref("public/" + uid)
    );
  },
  computed: {
    searchMark() {
    ...
    getBasicInfo() {
      ...
      if (this.typeProfile == "public") {
        const publicInfo = { ...this.publicInfo };
        const share = publicInfo["share"];
        ...
        return merge;
      } else if (this.typeProfile == "private") {
        const privateInfo = { ...this.privateInfo };
        const share = privateInfo["share"];
    ...
    getMarkShow() {
      let bangdiem = null;
      if (this.typeProfile == "public") {
        if (!this.publicInfo.share) return [];
        bangdiem = this.publicInfo.share.bangdiem;
      } else if (this.typeProfile == "private") {
        if (!this.privateInfo.share) return [];
        bangdiem = this.privateInfo.share.bangdiem;
      }
      if (!bangdiem) return [];
      const result = [];
    ...
    getBasicInfoShow() {
      ...
      if (this.typeProfile == "public") {
        if (!this.publicInfo.share) return [];
        basicinfo = this.publicInfo.share.basicinfo;
      } else if (this.typeProfile == "private") {
        if (!this.privateInfo.share) return [];
        basicinfo = this.privateInfo.share.basicinfo;
      }
      if (!basicinfo) return [];
      const result = [];
      for (let key in basicinfo) {
  ...
  methods: {
    async changeNickName(id) {
      ...
      await db.ref("public/" + id).update({
        nick_name: editName,
        lastChangeName: firebase.database.ServerValue
          .TIMESTAMP
      });
      this.isEditName = "";
    },
